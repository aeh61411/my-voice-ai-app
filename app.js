// File: app.js (Corrected and Final Version)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const el = { /* ... (same as before, no changes needed here) ... */
    ...
    // BUG FIX: Make logout UI update immediately.
    ...
    async function loadUserProfile() { /* ... (same as before, no changes needed here) ... */ 
    ...
    el.avatarUploadInput.addEventListener('change', async () => { /* ... (same as before, no changes) ... */
    ...
    el.saveProfileBtn.addEventListener('click', async () => { /* ... (same as before, no changes) ... */
    ...
    // BUG FIX: This function now works correctly.
    el.brainstormBtn.addEventListener('click', async () => {
        ...
        try {
            // Updated to send the correct payload structure
    ...
    function speakText(text, onEndCallback) {
        ...
        utterance.onend = onEndCallback; // The magic happens here: the next action waits until speech is done.
    ...
    async function getAIChatResponse() {
        ...
        try {
            // Updated to send history and system instructions separately for efficiency
            ...
            // This callback ensures recognition only starts AFTER the AI finishes speaking.
            speakText(aiText, () => {
    ...
    if (SpeechRecognition) {
        ...
        chatRecognition.onresult = (event) => {
            ...
            getAIChatResponse(); // Get AI response after user speaks
        };
        chatRecognition.onstart = () => { el.chatStatus.textContent = "Listening..."; };
        chatRecognition.onerror = (event) => { if (event.error !== 'no-speech') { console.error('Speech recognition error:', event.error); el.chatStatus.textContent = "Mic error. Try again."; } };
        ...
        el.startChatBtn.addEventListener('click', () => {
            if (chatState.isChatting) {
                ...
                chatState.history.push({ role: 'model', parts: [{ text: firstAIResponse }] });
                speakText(firstAIResponse, () => { if (chatState.isChatting) chatRecognition.start(); });
            }
        });
    }
});