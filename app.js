document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- GLOBAL SETUP & ELEMENT GETTING ---
    const { createClient } = supabase; const _supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY); const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; const synthesis = window.speechSynthesis; let currentUser = null;
    const el = {
    ...
    el.tabButtons.forEach(btn => { btn.onclick = () => showTab(btn.dataset.target); });
    ...
    async function loadUserProfile() { if (!currentUser) return; const { data, error } = await _supabase.from('profiles').select('username, avatar_url').eq('id', currentUser.id).single(); if (error && error.code !== 'PGRST116') console.error('Error loading profile:', error); const username = data?.username || currentUser.email.split('@')[0]; const avatarUrl = data?.avatar_url || 'https://i.imgur.com/8mUOF1k.png'; /* <-- NEW DEFAULT AVATAR */ el.userInfo.textContent = username; el.headerAvatar.src = avatarUrl; el.profileAvatarPreview.src = avatarUrl; el.usernameInput.value = username; }
    
    el.uploadAvatarBtn.onclick = () => el.avatarUploadInput.click();
    el.avatarUploadInput.onchange = async () => { if (!currentUser || !el.avatarUploadInput.files || el.avatarUploadInput.files.length === 0) return; const file = el.avatarUploadInput.files[0]; const filePath = `${currentUser.id}/${Date.now()}_${file.name}`; el.uploadAvatarBtn.textContent = 'UPLOADING...'; try { const { error: uploadError } = await _supabase.storage.from('avatars').upload(filePath, file, { upsert: true }); if (uploadError) throw uploadError; const { data: { publicUrl } } = _supabase.storage.from('avatars').getPublicUrl(filePath); await _supabase.from('profiles').upsert({ id: currentUser.id, avatar_url: publicUrl }); alert('Picture updated!'); await loadUserProfile(); } catch (error) { alert('Error uploading picture!'); console.error(error); } finally { el.uploadAvatarBtn.textContent = 'Upload New Picture'; }};
    el.saveProfileBtn.onclick = async () => { if (!currentUser) return; const newUsername = el.usernameInput.value; el.saveProfileBtn.textContent = 'SAVING...'; try { const { error } = await _supabase.from('profiles').upsert({ id: currentUser.id, username: newUsername }); if (error) throw error; alert('Username saved!'); await loadUserProfile(); } catch (error) { alert('Error saving username!'); console.error(error); } finally { el.saveProfileBtn.textContent = 'Save Profile'; }};
    ...
    // --- TAB 2: IDEA SPARK ---
    el.brainstormBtn.onclick = async () => { const topic = el.brainstormInput.value; if (!topic) { alert('Please enter a topic!'); return; } el.aiResponseDiv.innerHTML = '🧠 Generating innovative ideas...'; const prompt = `You are an expert business and product strategist. A user wants innovative ideas for the topic: "${topic}". Provide 3 distinct and actionable ideas. For each idea, provide a catchy name, a one-sentence summary, and 2-3 key features. Format the output clearly and professionally using markdown for bolding and bullet points.`; try { const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${GEMINI_API_KEY}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ "contents": [{ "parts": [{ "text": prompt }] }] }) }); if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`); const data = await response.json(); el.aiResponseDiv.textContent = data.candidates[0].content.parts[0].text; } catch (error) { console.error("Error:", error); el.aiResponseDiv.textContent = 'AI Error. Check API key and billing.'; } };
